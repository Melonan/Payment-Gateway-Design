...
def _make_invalid_request(**overrides) -> PostPaymentRequest:
    """
...
class TestValidatePaymentRequest:
    ...
    def test_card_number_too_short(self):
        """ less than 14 digits """
        request = _make_invalid_request(card_number="1234567890123")  # 13 位
        errors = validate_payment_request(request)
        assert any("card_number" in e for e in errors)

    def test_card_number_too_long(self):
        """ more than 19 digits"""
        request = _make_invalid_request(card_number="12345678901234567890")  # 20 位
        errors = validate_payment_request(request)
        assert any("card_number" in e for e in errors)

    def test_card_number_contains_letters(self):
        """invalid char, should be all digits"""
        request = _make_invalid_request(card_number="1234abcd56789012")
        errors = validate_payment_request(request)
        assert any("card_number" in e for e in errors)

    def test_card_number_14_digits_is_valid(self):
        """valid card num, with 14 digits """
        request = _make_invalid_request(card_number="12345678901234")
        errors = validate_payment_request(request)
        assert errors == []
    ...
    def test_expiry_month_zero(self):
        """month is 0, illegal"""
        request = _make_invalid_request(expiry_month=0)
        errors = validate_payment_request(request)
        assert any("expiry_month" in e for e in errors)

    def test_expiry_month_13(self):
        """month is 13, illegal"""
        request = _make_invalid_request(expiry_month=13)
        errors = validate_payment_request(request)
        assert any("expiry_month" in e for e in errors)

    def test_expiry_month_1_is_valid(self):
        """Jan, legal"""
        request = _make_invalid_request(expiry_month=1)
        errors = validate_payment_request(request)
    ...
    def test_expiry_in_past_year(self):
        """expired date"""
        request = _make_invalid_request(expiry_year=2020, expiry_month=6)
        errors = validate_payment_request(request)
    ...
    def test_expiry_current_month(self):
        ...
        now = datetime.utcnow()
        request = _make_invalid_request(expiry_year=now.year, expiry_month=now.month)
        errors = validate_payment_request(request)
        assert any("future" in e for e in errors)
    ...
    def test_expiry_next_month_is_valid(self):
        ...
        request = _make_invalid_request(expiry_year=next_year, expiry_month=next_month)
        errors = validate_payment_request(request)
    ...
    def test_amount_zero(self):
        """amount == 0 , invalid"""
        request = _make_invalid_request(amount=0)
        errors = validate_payment_request(request)
        assert any("amount" in e for e in errors)

    def test_amount_negative(self):
        """amount < 0, invalid"""
        request = _make_invalid_request(amount=-100)
        errors = validate_payment_request(request)
        assert any("amount" in e for e in errors)

    def test_amount_123_is_valid(self):
        """vaild"""
        request = _make_invalid_request(amount=123)
        errors = validate_payment_request(request)
